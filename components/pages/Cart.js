class Page{
    ...
    create(){
        ...
        if(document.cookie.length == 0){
            this.#element.innerHTML = `<h2>Cart is empty(</h2>
            <div >
                <img class="emptyCart" src="/components/image/emptyCart.jpg">
            </div>`    
        }else{
            ...
            localData.forEach(item =>{
                if (cookie.includes(String(item.id))){
                    ...
                    let productTitleElem = document.createElement('td');
                    productTitleElem.innerHTML = item.title;
                    productTitleElem.classList.add('main__title')
                    ...
                    btnPlus.addEventListener('click', ()=>{
                        productPriceElem.innerHTML =  Math.round(parseInt(productPriceElem.innerHTML) + item.price) + "$";
                        this.totalPrice();
                        thing.innerHTML++;
                    });
                    ...
                    productTrElem.append(productTitleElem, productImageElem, productAmountElem, productPriceElem,)
    ...
    totalPrice(){
        ...
        testPrice.forEach(item =>{
            summ.innerHTML = parseInt(summ.innerHTML) + parseInt(item.innerHTML) + "$";
        })
